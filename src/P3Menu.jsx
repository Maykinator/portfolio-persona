...
import meuCurriculo from "./assets/curriculo.pdf";
import { playEnter, playMove, playConfirm } from "./soundManager";

const ITEMS = [
  { id: "about",   label: "SOBRE MIM",      page: "about",   fontSize: 80, offsetX: 0,  offsetY: 0,  skew: -6,  skewY: 10  },
  { id: "resume",  label: "RESUMO",        page: "resume",  fontSize: 66, offsetX: 20, offsetY: 8,  skew: -11, skewY: -10 },
  { id: "github",  label: "LINK GITHUB",   page: "https://github.com/Maykinator",  fontSize: 68, offsetX: 8, offsetY: 6,  skew: 0, skewY: -4  },
  { id: "socials", label: "PROJETOS",       page: "socials", fontSize: 74, offsetX: 16, offsetY: 8,  skew: -3,  skewY: 5   },
  { id: "sideproj",label: "CURRICULO", page: meuCurriculo,fontSize: 56, offsetX: 10, offsetY: 6,  skew: -4,  skewY: 7   },
];
...
const handleAction = (target) => {
  playConfirm();
    if (target.includes(".pdf") || target.startsWith("http")) {
...
const activate = (idx) => {
  ...
  useEffect(() => {
    ...
    const visualTimer = setTimeout(() => setMounted(true), 1000);
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === "Enter") {
        ...
        handleAction(ITEMS[active].page); 
      }
    };
