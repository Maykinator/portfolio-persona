...
const ITEMS = [
  ...
  {
    id: "design", label: "DESIGN", handle: "Identidade Visual", href: "#", icon: "🎨", barIcon: icon3, bars: 2, newBars: [0], 
...
export default function Socials() {
  ...
  const handleActivateSub = (newIndex) => {
    if (activeInfoBar !== newIndex) {
      playMove();
  ...
  useEffect(() => {
    const onKey = (e) => {
      if (focus === "left") {
        if (e.key === "ArrowUp")    handleActivateMain(Math.max(0, active - 1));
        if (e.key === "ArrowDown")  handleActivateMain(Math.min(ITEMS.length - 1, active + 1));
        if (e.key === "ArrowRight") { 
          playConfirm();
          setFocus("right"); 
          setActiveInfoBar(0); 
        ...
        if (e.key === "ArrowUp")   handleActivateSub(Math.max(0, activeInfoBar - 1));
        if (e.key === "ArrowDown") handleActivateSub(Math.min(barCount - 1, activeInfoBar + 1));
        if (e.key === "ArrowLeft") {
          playConfirm(); 
          setFocus("left");
        }
        if (e.key === "Enter") {
          playConfirm();
          const targetUrl = ITEMS[active].projectUrls[activeInfoBar];
